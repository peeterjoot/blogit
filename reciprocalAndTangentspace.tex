...
Let's apply this blade dot product form of the reciprocal frame vectors to some non-orthogonal examples.  For the first example, consider an elliptical parameterization
...
\end{dmath}
which shows that these vectors are orthogonal in the limiting circular case, where the eccentricity \( \epsilon \) goes to one, and at the points \( \theta = n \pi/2 \).
The area element is
