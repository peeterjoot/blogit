...
An alternative, one that is possibly more computationally effecient, is using matrix algebra to perform the same computation.  Considering the two parameter case again, we can write
...
\begin{equation}\label{eqn:reciprocalAndTangentspace:1200}
   D = X^\T G X,
\end{equation}
where \( G \) is the metric matrix for the space.  For most of physics, there are really only two metrics of interest.  The first is the identity matrix \( G = I \), which we use for Euclidean spaces, and \( G = \pm \diag(1,-1,-1,-1) \), the metric for special relativity.

This gives us
\begin{equation}\label{eqn:reciprocalAndTangentspace:1220}
\begin{aligned}
   \Bx^a &= X \lr{ X^\T G X }^{-1} \begin{bmatrix} 1 \\ 0 \end{bmatrix} \\
   \Bx^b &= X \lr{ X^\T G X }^{-1} \begin{bmatrix} 0 \\ 1 \end{bmatrix}.
\end{aligned}
\end{equation}
...
\end{aligned}
\end{equation}
We get the same results, but it was a lot trickier to compute than the vector-bivector dot products or matrix inverses, and that computation only seemed possible because we could solve for \( r, \theta \) in this specific case.

What is really cool, is that we don't actually have to be able to solve for the parameters as functions of the coordinates, since we can use the same implicit differentiation methods used above in a more systematic fashion.  Let's use the two variable case again to illustrate the method.  The generalization will be clear.
