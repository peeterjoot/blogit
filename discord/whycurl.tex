...
Why we pick \( \phi \) and \( \BA \) as our scalar and vector potentials is covered in many books.
For example, see
\begin{enumerate}
\item \S 6.4 Vector and Scalar Potentials, in \citep{jackson1975cew},
\item \S 18-6 potentials and the wave equation in \citep{feynman1963flpII:MaxwellEquations}, and
\item \S 10.1 The potential formulation in \citep{griffiths1999introduction}.
\end{enumerate}
The basic idea in all of those is that we are looking for representations of the fields that automatically satisfy the pair of source free Maxwell's equations
...
The approach used in all of those to use utilize the relations
...
\end{aligned}
\end{equation}
where \( \Bf \) is a vector, and \( \chi \) is a scalar.
...
\section{Potentials if we have magnetic sources.}
The multivector form of Maxwell's equation is
\begin{equation}\label{eqn:whycurl:760}
\lr{ \spacegrad + \partial_0 } F = J_{0,1} + J_{2,3},,
\end{equation}
where \( F = \BE + I c \BB \), \( J_{0,1} \) is a multivector with grades \( 0,1 \), including conventional electric charge density and current density contributions, and \( J_{2,3} \) has grades \( 2, 3\) and includes the fictitous magnetic sources that we use in antenna and microwave engineering.

We've found a potential representation for \( F \) for the case where \( J_{2,3} = 0 \).  Now let's consider the \( J_{0,1} = 0 \) case.  In this case, the source free equations are
\begin{equation}\label{eqn:whycurl:780}
\gpgrade{ \lr{ \spacegrad + \partial_0 } F }{0,1} = 0.
\end{equation}
Expanding that grade selection, we have
\begin{equation}\label{eqn:whycurl:800}
\begin{aligned}
\gpgrade{ \lr{ \spacegrad + \partial_0 } F }{0,1}
&=
\gpgrade{ \lr{ \spacegrad + \partial_0 } \lr{ \BE + I c \BB } }{0,1} \\
&=
\end{aligned}
\end{equation}
