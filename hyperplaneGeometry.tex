...
\generatetitle{Equation of a hyperplane, and shortest distance between two hyperplanes.}
%\chapter{Equation of a hyperplane, and shortest distance between two hyperplanes.}
...
\begin{proof}
Let \( \beta = \setlr{ \fcap_1, \cdots \fcap_{N-1} } \) be an orthonormal basis for the hyperplane normal to \( \ncap \), and \( \Bd = d \ncap \) be the vector in that hyperplane, closest to the origin, as illustrated in \cref{fig:hyperplane:hyperplaneFig1}.  Then the hyperplane \( d \) distant from the origin with normal \( \ncap \) has the parametric representation
\imageFigure{../figures/blogit/hyperplaneFig1}{\R{3} plane with normal \( \ncap \).}{fig:hyperplane:hyperplaneFig1}{0.4}
\begin{equation}\label{eqn:hyperplaneGeometry:40}
\Bx(a_1, \cdots, a_{N-1}) = d \ncap + \sum_{i = 1}^{N-1} a_i \fcap_i.
\end{equation}
...
\end{aligned}
\end{equation}
where \( \ncap = (1,-1,2)/\sqrt{6} \), as illustrated in \cref{fig:hyperplane:hyperplaneFig2}.
\imageFigure{../figures/blogit/hyperplaneFig2}{The two planes.}{fig:hyperplane:hyperplaneFig2}{0.4}
