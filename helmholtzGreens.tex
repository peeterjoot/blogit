...
The great mathematical physics book \citep{byron1992mca} derives the Green's function for the 3D Helmholtz (wave equation) operator.  It turns out that unless you are are intimate with Bessel functions, the 2D generalization of that result is difficult.
...
We can split the convolution kernel a ``causal'' part, where only the spatiallly-``past'' values of \( V \) contribute, and an ``acausal'' part
...
Let's show that the outgoing Green's function has the desired delta function semanitics.  That is
