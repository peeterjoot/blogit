...
\newcommand{\mybinom}[2]{{{#1}\choose{#2}}}
...
{\sqrt{5}}^k
-
\sum_{k = 0}^n \mybinom{n}{k} (-\sqrt{5})^k
=
2 \sum_{1 \le k \le n, \mbox{$k$ is odd}} \mybinom{n}{k} (\sqrt{5})^k
=
2 \sqrt{5} \sum_{m = 0}^{\floor*{(n-1)/2}} \mybinom{n}{2 m + 1} 5^m,
\end{dmath}
% k = 2 m + 1, m = 0, 2 m + 1 <= n ; m <= (n-1)/2
so (for any \( n > 0 \),
\begin{dmath}\label{eqn:fibonacci:240}
F_n = 
\inv{2^{n-1}} \sum_{m = 0}^{\floor*{(n-1)/2}} \mybinom{n}{2 m + 1} 5^m.
\end{dmath}
...
It is still somewhat remarkable seeming that there is always a perfect cancelation of all the factors of two in the basement, but we can see easily why the end result, for any \( n \) has no \( \sqrt{5} \) terms in it.
%}
\EndArticle
