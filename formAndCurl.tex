...
\spacegrad \cross (\BF \cross \BG) &= (\spacegrad \cdot \BF) \cdot \BG - \BF \cdot (\spacegrad \cross \BG),
\end{aligned}
\end{equation}
...
\makelemma{}{lemma:formAndCurl:840}{
Let \( \omega = f_i dx_i \) be a one-form.  The exterior derivative of \( d \omega \) can be expanding into a Jacobian form
\begin{equation*}
d\omega
=
\sum_{i < j} \lr{
...
Now let's look at the equivalence between the exterior derivative of a two-form with the curl.  Let's write
\begin{equation}\label{eqn:formAndCurl:320}
\eta = \sum_{ij} f_{ij} dx_i \wedge dx_j
\end{equation}
...
\begin{equation}\label{eqn:formAndCurl:360}
...
\end{aligned}
\end{equation}
Again, we see an exact coorespondence with the exterior derivative \( d \eta \) of a two-form, and the curl \( \spacegrad \wedge B \), of a 2-blade.
...
It shouldn't be suprising that this is the equivalent of \( d^2 A = 0 \) from differential forms.  Let's prove this, first considering the 0-blade case
\begin{proof}
...
We have a couple chain rule identities in the article.  Here is the GA equivalent of that, and its corollaries
...
We have some chain rule identities left in the article to verify and to find GA equivalents of.  Before doing so, we need a couple miscellaneous identities relating triple corss products to wedge-dots.
...
We have a couple identities in the article that all have expressions like \( \Ba \cdot \lr{ \Bb \cross \Bc } \), so we want to figure out a natural GA representation of such an expression.  That is
\makelemma{Dual of triple wedge.}{lemma:formAndCurl:660}{
If \( \Ba, \Bb, \Bc \) are vectors, then
\begin{equation*}
