...
In this form, with the multivector charge and current density \( J \), this looks unfamiliar.  Let's plug in:
...
\end{equation}
If we zero the fictitious sources, setting \( \rho_\txtm = 0, \BM = 0 \), these are the Jefimenko's equations, where all times and time derivatives are with respect to the retarded time \( t_r = r - \Abs{\Bx - \Bx'}/c \).

Having computed the advanced Green's function, we can also write out the advanced time equivalent of Jefimenko's equations.
