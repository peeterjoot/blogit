...
My favorite book on mathematical physics derives the Green's function for the 3D Helmholtz (wave equation) operator.  I tried to derive the 2D Green's function the same way and had trouble.  Here I'll try again, starting with the easy 1D and 3D cases, and then see if I can do the trickier 2D case again.
...
It is interesting that the specific solution above has equal causal and acausal contributions.  Such a solution (outside of QFT) is generally undesirable.  We can construct a specific solution that is either causal or acasual by picking just one of the integrals above, instead of averaging.  For example, let
...
\end{equation}
but must then use the boundary value constraints to determine the desired form of the Green's function, using the two degrees of freedom to do so.
