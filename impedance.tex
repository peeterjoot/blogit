...
v_s(t) = L \frac{di}{dt}.
\end{equation}
Again, we can write \( v_s(t) = \Real \lr{ V e^{j\omega t} } \), and assume that \( i = \Real{ I e^{j\omega t} } \).  We then require
...
\end{aligned}
\end{equation}
The complicated time response for this system is reduced to a trivial voltage divider calculation.  Imagine how messy it would be to try to solve this sytem using the Green's function methods that we used above!
